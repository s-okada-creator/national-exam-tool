...
class ReportGenerator:
    ...
    def generate_markdown_report(self, session_data: Dict[str, Any]) -> str:
        ...
        for i, answer_data in enumerate(answers, 1):
            ...
            # あなたの回答を選択肢番号とテキストで表示
            if user_answer is None:
                user_answer_text = "未回答"
            elif isinstance(user_answer, list):
                user_answer_parts = []
                for ans in user_answer:
                    choice_text = choices.get(str(ans), f"選択肢{ans}（データなし）")
                    user_answer_parts.append(f"{ans}: {choice_text}")
                user_answer_text = ", ".join(user_answer_parts) if user_answer_parts else "未回答"
            else:
                choice_text = choices.get(str(user_answer), f"選択肢{user_answer}（データなし）")
                user_answer_text = f"{user_answer}: {choice_text}"
            ...
            if isinstance(correct_answer, list):
                ...
                for ans in correct_answer:
                    choice_text = choices.get(str(ans), f"選択肢{ans}（データなし）")
                    correct_answer_parts.append(f"{ans}: {choice_text}")
                correct_answer_text = ", ".join(correct_answer_parts) if correct_answer_parts else "データなし"
            else:
                choice_text = choices.get(str(correct_answer), f"選択肢{correct_answer}（データなし）")
                correct_answer_text = f"{correct_answer}: {choice_text}"
    ...
    def generate_pdf_report(self, session_data: Dict[str, Any]) -> BytesIO:
        ...
        for i, answer_data in enumerate(answers, 1):
            ...
            story.append(Paragraph(f"あなたの回答: {user_answer_text}", normal_style))
            ...
            story.append(Paragraph(f"正解: {correct_answer_text}", normal_style))
            ...
            if not is_correct and question.get('explanation'):
                ...
                if explanation:
                    ...
                    story.append(Paragraph("<b>解説</b>:", normal_style))
                    # 解説を複数行に対応させる
                    explanation_lines = explanation.split('\n')
                    for line in explanation_lines:
                        if line.strip():
